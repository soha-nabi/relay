...
export class WebRTCConnection extends BaseConnection {
     ...
     */
    async sendRequiredMessage(message) {
        if (!this.isConnected || !this.room.localParticipant) {
            throw new Error("Cannot send message: room not connected or no local participant");
        }
        this.handleOutgoingMessage(message);
        await this.publishMessage(message);
    }
        ...
        try {
            ...
            if (!publication?.track) {
                ...
                try {
                    ...
                    const recoveredTrack = this.room.localParticipant.getTrackPublication(Track.Source.Microphone)?.track;
                    if (recoveredTrack) {
    ...
    async enableMicrophone(inputDeviceId) {
        if (!inputDeviceId) {
    ...
    createMicrophoneTrack(deviceId) {
