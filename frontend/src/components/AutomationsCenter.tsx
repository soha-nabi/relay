...
export default function AutomationsCenter() {
  ...
  return (
    <div style={{ background: 'white', borderRadius: '24px', padding: '36px', border: '1px solid #e2e8f0', boxShadow: '0 4px 24px rgba(15,23,42,0.04)' }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '13px' }}>
            <thead>
              <tr style={{ background: '#f8fafc', borderBottom: '1px solid #e2e8f0' }}>
                <th style={{ padding: '14px 20px', fontWeight: 600, color: '#64748b' }}>Name</th>
                <th style={{ padding: '14px 20px', fontWeight: 600, color: '#64748b' }}>Trigger</th>
                <th style={{ padding: '14px 20px', fontWeight: 600, color: '#64748b' }}>Action</th>
                <th style={{ padding: '14px 20px', fontWeight: 600, color: '#64748b' }}>Status</th>
                <th style={{ padding: '14px 20px', fontWeight: 600, color: '#64748b' }}>Executed</th>
                <th style={{ padding: '14px 20px', fontWeight: 600, color: '#64748b', textAlign: 'right' }}>Actions</th>
              </tr>
            </thead>
            <tbody>
              {automations.map((auto) => (
                <tr key={auto.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                  <td style={{ padding: '16px 20px', fontWeight: 600, color: '#0f172a' }}>{auto.name}</td>
                  <td style={{ padding: '16px 20px', color: '#475569' }}>
                    <code style={{ background: '#f1f5f9', padding: '3px 8px', borderRadius: '6px', fontSize: '12px' }}>{auto.trigger}</code>
                  </td>
                  <td style={{ padding: '16px 20px', color: '#475569' }}>{auto.actions?.[0]?.type || 'Smart Retry'}</td>
                  ...
                  <td style={{ padding: '16px 20px', color: '#64748b', fontWeight: 600 }}>{auto.execution_count ?? auto.times_triggered ?? 0} times</td>
      ...
      <div style={{ marginTop: '36px', paddingTop: '28px', borderTop: '1px solid #f1f5f9' }}>
        <p style={{ fontSize: '12px', fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.06em', margin: '0 0 6px' }}>
          Interactive Automation Simulator
        </p>
        <h4 style={{ fontSize: '16px', fontWeight: 700, color: '#0f172a', margin: '0 0 12px' }}>
        ...
        <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
          <input
            ...
            style={{ padding: '10px 14px', borderRadius: '10px', border: '1px solid #cbd5e1', fontSize: '13px', width: '220px' }}
          />
          <button
            onClick={handleTestTrigger}
            disabled={loading}
            style={{ background: '#2563eb', color: 'white', border: 'none', borderRadius: '10px', padding: '10px 18px', fontSize: '13px', fontWeight: 600, cursor: 'pointer' }}
          >
            Trigger Event
          </button>
        </div>
        ...
        {triggerResult && (
          <div style={{ marginTop: '16px', background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '12px', padding: '14px', fontSize: '13px' }}>
      ...
      {showModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.6)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
          <div style={{ width: '100%', maxWidth: '520px', background: 'white', borderRadius: '24px', padding: '36px', border: '1px solid #e2e8f0', boxShadow: '0 20px 50px rgba(0,0,0,0.2)' }}>
            <h3 style={{ fontSize: '20px', fontWeight: 800, color: '#0f172a', margin: '0 0 20px' }}>Create Recovery Automation</h3>
            ...
            <form onSubmit={handleCreate} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '10px', marginTop: '12px' }}>
                ...
                <button
                  type="submit"
                  style={{ background: '#0f172a', color: 'white', border: 'none', borderRadius: '10px', padding: '10px 22px', fontSize: '13px', fontWeight: 600, cursor: 'pointer' }}
                >
                  Save Automation
                </button>
              </div>
