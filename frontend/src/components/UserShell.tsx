...
import { Home, CreditCard, HelpCircle, User as UserIcon, ArrowRight, CheckCircle2, AlertTriangle, Smartphone, ShieldCheck } from 'lucide-react';
...
export default function UserShell() {
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto', minHeight: '80vh' }}>
      ...
      <div style={{ display: 'flex', gap: '8px', marginBottom: '28px', background: 'white', padding: '6px', borderRadius: '16px', border: '1px solid #e2e8f0' }}>
        {[
          { id: 'home', label: 'Home', icon: <Home size={15} /> },
          { id: 'payments', label: 'My Payments', icon: <CreditCard size={15} /> },
          { id: 'recovery', label: 'Payment Recovery', icon: <HelpCircle size={15} /> },
      ...
      {error && (
        <div style={{ background: '#fef2f2', border: '1px solid #fecaca', borderRadius: '12px', padding: '14px', color: '#b91c1c', fontSize: '13px', marginBottom: '20px' }}>
          ⚠️ {error}
        </div>
      )}
      ...
      {activeTab === 'home' && !showCheckout && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          {selectedTxn ? (
            <div style={{ background: '#fffbeb', border: '1px solid #fef3c7', borderRadius: '24px', padding: '32px', boxShadow: '0 4px 20px rgba(245,158,11,0.06)' }}>
              ...
              <h2 style={{ fontSize: '28px', fontWeight: 800, color: '#0f172a', letterSpacing: '-0.02em', margin: '0 0 8px' }}>
              ...
              <div style={{ display: 'flex', gap: '12px' }}>
                ...
                >
                  Continue Payment <ArrowRight size={16} />
                </button>
              </div>
            </div>
          ) : (
            <div style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: '24px', padding: '32px', textAlign: 'center' }}>
              <CheckCircle2 size={40} style={{ color: '#16a34a', margin: '0 auto 12px' }} />
              <h3 style={{ fontSize: '20px', fontWeight: 700, color: '#166534', margin: '0 0 6px' }}>All Payments Up to Date</h3>
              <p style={{ fontSize: '14px', color: '#15803d', margin: 0 }}>You have no outstanding payment recovery requests.</p>
            </div>
          )}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px' }}>
            <div style={{ background: 'white', padding: '20px', borderRadius: '16px', border: '1px solid #e2e8f0' }}>
              <span style={{ fontSize: '12px', color: '#64748b', fontWeight: 600 }}>Total Transactions</span>
              <div style={{ fontSize: '22px', fontWeight: 800, color: '#0f172a', marginTop: '4px' }}>{data?.summary.total_transactions || 4}</div>
            </div>
            <div style={{ background: 'white', padding: '20px', borderRadius: '16px', border: '1px solid #e2e8f0' }}>
              <span style={{ fontSize: '12px', color: '#64748b', fontWeight: 600 }}>Successful</span>
              <div style={{ fontSize: '22px', fontWeight: 800, color: '#16a34a', marginTop: '4px' }}>{data?.summary.successful_payments || 2}</div>
            </div>
            <div style={{ background: 'white', padding: '20px', borderRadius: '16px', border: '1px solid #e2e8f0' }}>
              <span style={{ fontSize: '12px', color: '#64748b', fontWeight: 600 }}>Pending / Failed</span>
              <div style={{ fontSize: '22px', fontWeight: 800, color: '#d97706', marginTop: '4px' }}>{data?.summary.failed_payments || 2}</div>
            </div>
          </div>
        </div>
      )}
      ...
      {activeTab === 'payments' && (
        <div style={{ background: 'white', borderRadius: '20px', padding: '28px', border: '1px solid #e2e8f0' }}>
          <h3 style={{ fontSize: '18px', fontWeight: 800, color: '#0f172a', margin: '0 0 20px' }}>Payment History</h3>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {(data?.transactions || []).map((t) => (
              <div key={t.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px', borderRadius: '14px', background: '#f8fafc', border: '1px solid #f1f5f9' }}>
                <div>
                ...
                <div style={{ textAlign: 'right' }}>
                  <div style={{ fontSize: '16px', fontWeight: 800, color: '#0f172a' }}>{money(t.amount)}</div>
                  {t.recovery_available && (
                    <button
                      ...
                      style={{ background: 'transparent', border: 'none', color: '#2563eb', fontSize: '12px', fontWeight: 700, cursor: 'pointer', padding: 0, marginTop: '4px' }}
                    >
                      Restore Payment →
                    </button>
                  )}
      ...
      {activeTab === 'recovery' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          <div style={{ background: 'white', borderRadius: '20px', padding: '28px', border: '1px solid #e2e8f0' }}>
            <h3 style={{ fontSize: '18px', fontWeight: 800, color: '#0f172a', margin: '0 0 16px' }}>Self-Service Recovery Assistance</h3>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              {instructions.map((inst, idx) => (
                <div key={idx} style={{ background: '#f8fafc', padding: '18px', borderRadius: '14px', border: '1px solid #e2e8f0' }}>
                  <div style={{ fontSize: '14px', fontWeight: 700, color: '#0f172a', marginBottom: '4px' }}>{inst.title}</div>
                  <p style={{ fontSize: '13px', color: '#64748b', margin: '0 0 12px', lineHeight: 1.5 }}>{inst.description}</p>
                  <button
                    onClick={() => setShowCheckout(true)}
                    style={{ background: 'white', border: '1px solid #cbd5e1', borderRadius: '8px', padding: '6px 14px', fontSize: '12px', fontWeight: 600, color: '#2563eb', cursor: 'pointer' }}
                  >
                    {inst.action} →
                  </button>
                </div>
          ...
          {/* Support Helpline */}
          {support && (
            <div style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: '16px', padding: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                <span style={{ fontSize: '12px', fontWeight: 700, color: '#166534', textTransform: 'uppercase' }}>Need Help?</span>
                <div style={{ fontSize: '14px', fontWeight: 600, color: '#14532d', marginTop: '2px' }}>{support.email} · {support.helpline}</div>
              </div>
      ...
      {activeTab === 'profile' && (
        <div style={{ background: 'white', borderRadius: '20px', padding: '28px', border: '1px solid #e2e8f0' }}>
          <h3 style={{ fontSize: '18px', fontWeight: 800, color: '#0f172a', margin: '0 0 20px' }}>User Account Profile</h3>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', paddingBottom: '12px', borderBottom: '1px solid #f1f5f9' }}>
              ...
              <span style={{ fontWeight: 600, color: '#0f172a', fontSize: '13px' }}>{data?.user_profile?.email || 'user@relay.local'}</span>
            </div>
